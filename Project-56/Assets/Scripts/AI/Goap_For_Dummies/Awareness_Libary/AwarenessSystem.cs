...
public class AwarenessSystem : MonoBehaviour {
    ...
    private void Start() {
        visionSensor = gameObject.GetComponent<CoffinVisionSensor>();
    ...
    public void OnSuspicious(TrackedTarget target) {
        if (isDebug && target.stimulator) Debug.Log(gameObject.name + " OnSuspicious(" + target.stimulator.name + ")");
        gAgent.OnSuspicious(target);
    }

    public void OnLostSuspicion(TrackedTarget target) {
        if (isDebug && target.stimulator)
            Debug.Log(gameObject.name + " OnLostSuspicion(" + target.stimulator.name + ")");
        gAgent.OnLostSuspicion(target);
    }

    public void OnDetected(TrackedTarget target) {
        if (isDebug && target.stimulator) Debug.Log(gameObject.name + " OnDetected(" + target.stimulator.name + ")");
        gAgent.OnDetected(target);
    }

    public void OnFullyDetected(TrackedTarget target) {
        if (isDebug && target.stimulator)
            Debug.Log(gameObject.name + " OnFullyDetected(" + target.stimulator.name + ")");
        gAgent.OnFullyDetected(target);
    }

    public void OnLostDetection(TrackedTarget target) {
        if (isDebug && target.stimulator)
            Debug.Log(gameObject.name + " OnLostDetection(" + target.stimulator.name + ")");
        gAgent.OnLostDetection(target);
    }

    public void OnFullyLost(TrackedTarget target) {
        if (isDebug && target.stimulator) Debug.Log(gameObject.name + " OnFullyLost(" + target.stimulator.name + ")");
