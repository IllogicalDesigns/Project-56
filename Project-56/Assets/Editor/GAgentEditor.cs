...
public class GAgentVisualEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (agent.gameObject.GetComponent<GAgent>().agentState != null) {
            GUILayout.Label("===== AgentState: ");
            foreach (KeyValuePair<string, object> p in
                     agent.gameObject.GetComponent<GAgent>()?.agentState?.GetStates()) {
        ...
        if (GWorld.Instance != null) {
            GUILayout.Label("===== WorldState: ");
            foreach (KeyValuePair<string, object> p in GWorld.Instance.GetWorld().GetStates()) {
                if (p.Value is TrackedTarget) {
                    var tracked = (TrackedTarget)p.Value;
                    if (tracked != null && tracked.stimulator != null)
                        GUILayout.Label(p.Key + " = " + tracked.stimulator.name);
